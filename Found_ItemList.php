...
$query = "SELECT * FROM found1";
$sql = $conn->query($query);  // prepare the
...
?>
     ...
     <h2 align="center"><mark>Display All   sing in and sing up Records</mark> </h2> -->
     <!-- <center><table border=1 cellspacing="7" whdth="45%">
        <tr>
            <th width="5%">ID</th>
        ...
        <!-- <php 
        // Advance the internal result pointer to the second row
        ...
        //         <td><=htmlspecialchars($result["founditem"]) ></td>
        //         <td><=htmlspecialchars($result["itemdescription"]) ></td>
        //         <td><=htmlspecialchars($result["place"]) ?</td>
        //         <td>
                //  <img src="<php echo htmlspecialchars($result["image"]); >" alt="" height='100px' width='100px' id=img />
                // </td>

               
               
            //     <td><a href='found-update.php?id=<= htmlspecialchars($result["id"])>'><input type="submit" value="Update" class="Update"></a>

            //     <a href='found-delete.php?id=<= htmlspecialchars($result["id"])>'><input type="submit" value="delete" class="delete" onclick = 'return checkdelete()'></a></td>
            // </tr>
//         <php
//         } -->
<!-- // >
  ...
  <?php while ($row = mysqli_fetch_assoc($sql)) { ?>
    ...
    <div class="image-container">
      ...
      <div class="caption">
        ...
        <p class="name"> <?php echo $row["founditem"]; ?></p>
        <p class="desc"> item description :-<?php echo $row["itemdescription"]; ?></p>
        <p class="place">place where you lost :-  <?php echo $row["place"]; ?></p>
      <center>
        <button class="add btn" data-id="<?php echo $row["id"]; ?>" onclick = found()>Yes! its mine</button>
        <a href='found-delete.php?id=<?= htmlspecialchars($row["id"])?>'><input type="submit" value="delete" class="delete btn" onclick = 'return checkdelete()'></a> 
         
         <a href='found-update.php?data-id=<?php echo $row["id"];?>'<input type="update" value="Update" class="btn add" ">Update</a>

        <script src="js_lost.js"></script>
        <script>function found() {
    alert('Are You Sure Want To ');
...
error_reporting(0);
 ...
 $lost = "SELECT * FROM found1 WHERE id='".$row['id']."'";
  $all_item = $conn ->query($found);
  ?>
